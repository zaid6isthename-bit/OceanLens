...
function parseKuehneResponse(json: any, bl: string): NormalizedShipment | null {
  if (!json) return null
  const vessel: any = {}
  ...
  const containers = json.containers || json.containerNumbers || json.equipmentList
  ...
  const normalized: NormalizedShipment = {
    ...
    pol: json.portOfLoading || json.origin,
    pod: json.portOfDischarge || json.destination,
    currentPort: json.currentPosition,
    nextPort: json.nextPortOfCall,
    eta: json.estimatedTimeOfArrival || json.eta,
...
const KuehneProvider: ForwarderProvider = {
  async fetchByBL(bl: string, userId?: string) {
    const cred = userId ? await prisma.apiCredential.findFirst({ where: { userId, provider: { contains: 'Kuehne' } } }) : null
    if (!cred) return null
    ...
    try {
      ...
      return parseKuehneResponse(json, bl)
    } catch (err) {
      return null
    }
  }
