...
function parseDBSchenkerResponse(json: any, bl: string): NormalizedShipment | null {
  if (!json) return null
  const vessel: any = {}
...
const DBSchenkerProvider: ForwarderProvider = {
  async fetchByBL(bl: string, userId?: string) {
    const cred = userId ? await prisma.apiCredential.findFirst({ where: { userId, provider: { contains: 'Schenker' } } }) : null
    if (!cred) return null
    ...
    try {
      ...
      return parseDBSchenkerResponse(json, bl)
    } catch (err) {
      return null
    }
  }
